...
const SignForm: FC<FormPropsType> = ({ type }) => {
    ...
    const { register, handleSubmit, formState: { errors, isValid } } = useForm<FormInputType>(getHookFormConfig(type, { email: location.state?.email }));
    ...
    const onSubmit: SubmitHandler<FormInputType> = async (data): Promise<void> => {
        try {
        ...
        >
            {
                signInputsConfig[type].map((input, index) => (
                    <FormInput register={register} {...input} error={errors[input.name]} key={input.name + index} />
                ))
            }
        </SignFormWrapper>
    );
